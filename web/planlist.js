(function(){
    ...
    // таблицы маркеров
    var ITEM_CHARS_STRING = planlist.ITEM_CHARS_STRING = "!\"#$%&\'()*+,-./0123456789:;<=>?@[\\]^_`{|}~";
    var MARKERS = planlist.MARKERS = ["absent", "done", "inwork", "question", "discard", "frozen", "bury", "idea", "future", "warning", "burning", "detail", "bullet"];
    var MARKER_CHARS = planlist.MARKER_CHARS = ['-', '+', '=|', '?', '/', '#', '_', '*', '^', '!', '%', ':', '.'];
    var CHAR_MARKERS = planlist.CHAR_MARKERS = [];
    for (var i=0; i<255; i++)  CHAR_MARKERS[i] = undefined;
    for (i=0; i<ITEM_CHARS_STRING.length; i++)  CHAR_MARKERS[ITEM_CHARS_STRING.charCodeAt(i)] = null;
    for (i=0; i<MARKER_CHARS.length; i++)  for (var j=0; j<MARKER_CHARS[i].length; j++)
        CHAR_MARKERS[MARKER_CHARS[i].charCodeAt(j)] = MARKERS[i];
    ...
    planlist.parse = function parse(content, parser)
    {
        ...
        for (var end=0; end<content.length; end++)  // end++ means skip '\n'
        {
            ...
            if (marker!==undefined && content.charCodeAt(i1+1)<=32)
            {
                ...
                //    добавить элемент
                parser.addItem(marker, content, i1);
